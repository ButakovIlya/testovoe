...
async def check_database_exists(engine: AsyncEngine, settings: Settings) -> bool:
    async with engine.connect() as connection:
        result = await connection.execute(
            CHECK_DATABASE_EXISTS_SQL, {"name": settings.db.name}
        )
...
def create_default_database_engine(settings: Settings) -> AsyncEngine:
    default_database_engine = settings.db.model_copy(
        update={"name": POSTGRES_DEFAULT_DB_NAME}
    )
    return create_async_engine(
        default_database_engine.dsn, isolation_level="AUTOCOMMIT"
    )
