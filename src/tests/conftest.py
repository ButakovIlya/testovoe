...
async def container(
    ...
    async with Container.lifespan(
        ...
        container.db.uow.override(
            providers.Factory(
                SqlAlchemyUnitOfWork, session_factory=alchemy_session_factory
            )
        )
...
async def http_client(
    app: FastAPI, http_client_base_url: str
) -> AsyncGenerator[AsyncClient, None]:
    transport = ASGITransport(app=app)
    async with AsyncClient(
        transport=transport, base_url=http_client_base_url
    ) as client:
        yield client
...
async def auth_http_client(
    ...
    async with AsyncClient(
        app=app, base_url=http_client_base_url, headers=headers
    ) as client:
        yield client
